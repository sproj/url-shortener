...
use api_gateway::{
    ...
    application::{
        app::App,
        config::Config,
        service::auth::refresh_token_cache::RefreshTokenCache,
        service::auth::refresh_token_cache_trait::{NoopRefreshTokenCache, RefreshTokenCacheTrait},
        state::SharedState,
    },
...
impl TestAppBuilder {
    ...
    pub async fn build(self) -> TestApp {
        ...
        let refresh_token_cache: Arc<dyn RefreshTokenCacheTrait> = if let Some(redis) = &self.redis
        {
            let conn = connect::connect(&redis.config).await.unwrap();
            app_builder = app_builder.with_redis(conn.clone());
            Arc::new(RefreshTokenCache::new(conn))
        } else {
...
fn config_from_db(db: &SharedTestDb) -> Config {
    let mut config = api_gateway::application::config::load().unwrap();
